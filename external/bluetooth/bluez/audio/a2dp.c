					...
					void *user_data)
{
...
#if defined(ANDROID) && defined(SEMC_BLUETOOTH_STE)
	bdaddr_t dst;
	...
	if (session) {
		if (new_state != AVDTP_STATE_STREAMING) {
			avdtp_get_peers(session, NULL, &dst);
			android_set_normal_priority(&dst);
		}

		if (old_state == AVDTP_STATE_OPEN &&
				new_state == AVDTP_STATE_STREAMING) {
			avdtp_get_peers(session, NULL, &dst);
			android_set_high_priority(&dst);
		}
	}
#endif
				...
				uint8_t *category, void *user_data)
{
	...
	if (dev)
		sink_set_protected(dev,scmst_protect);

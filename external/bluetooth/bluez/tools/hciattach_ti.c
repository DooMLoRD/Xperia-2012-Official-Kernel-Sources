...
#ifdef ANDROID
#define FIRMWARE_DIRECTORY	"/etc/firmware/"
#else
...
#define BD_ADDR_FILE "/data/etc/bluetooth_bdaddr"
...
static int load_bd_addr(const char* const filename, bdaddr_t* address) {
	FILE* fp;

	fp = fopen(filename, "r");
	if (!fp) {
		perror("can't open BDADDR file");
		return -1;
	}

	if (6 != fscanf(fp, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
					&address->b[0],
					&address->b[1],
					&address->b[2],
					&address->b[3],
...
static int set_bd_addr(int fd, bdaddr_t* address) {
	char cmd[10];
	...
	/* Write BDADDR command */
	cmd[0] = HCI_COMMAND_PKT;
	cmd[1] = 0x06;  /* CMD write BD Address */
	cmd[2] = 0xFC;  /* CMD write BD Address */
	cmd[3] = 0x06;  /* Length */

	cmd[4] = address->b[0];  /* Bluetooth address */
	cmd[5] = address->b[1];  /* ... */
	cmd[6] = address->b[2];  /* ... */
	cmd[7] = address->b[3];  /* ... */
	cmd[8] = address->b[4];  /* ... */
	...
	do {
		...
		if (n < 0) {
			perror("Failed to write init command (WRITE_BDADDR)");
			return -1;
		}
		if (n < (int)sizeof(cmd)) {
			fprintf(stderr, "failed to write complete address\n");
			return -1;
		}

		/* Read reply. */
		if (read_hci_event(fd, resp, 100) < 0) {
			perror("Failed to read response (WRITE_BDADDR)");
			return -1;
		}
...
static int brf_read_response(int fd, int hcill_installed)
{
				...
				int fd, struct termios *ti, int hcill_installed)
{
	...
	case ACTION_WAIT_EVENT:
		DPRINTF("R");
		ret = brf_read_response(fd, hcill_installed);
		break;
	case ACTION_SERIAL:
		DPRINTF("S");
...
int texas_init(int fd, struct termios *ti)
{
...

	{
		...
		res = load_bd_addr(BD_ADDR_FILE, &bdaddr);
		if (res >= 0) {
			res = set_bd_addr(fd, &bdaddr);
		}
		if (res < 0) {
			fprintf(stderr,"ERROR: Using default IEEE address\n");
		}
	}

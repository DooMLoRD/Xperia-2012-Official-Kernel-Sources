...
static char *rfcomm_flagstostr(uint32_t flags)
{
	...
	if (flags & (1 << RFCOMM_REUSE_DLC))
		strncat(str, "reuse-dlc ", sizeof(str) - strlen(str) - 1);

	if (flags & (1 << RFCOMM_RELEASE_ONHUP))
		strncat(str, "release-on-hup ", sizeof(str) - strlen(str) - 1);

	if (flags & (1 << RFCOMM_TTY_ATTACHED))
		strncat(str, "tty-attached", sizeof(str) - strlen(str) - 1);
...
{
	char src[18], dst[18], addr[40] = { '\0' };

	ba2str(&di->src, src); ba2str(&di->dst, dst);

	if (bacmp(&di->src, BDADDR_ANY) == 0)
		snprintf(addr, sizeof(addr), "%s", dst);
	else
		snprintf(addr, sizeof(addr), "%s -> %s", src, dst);

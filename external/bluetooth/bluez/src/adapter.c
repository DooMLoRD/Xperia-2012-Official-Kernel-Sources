...
static int adapter_up(struct btd_adapter *adapter, const char *mode)
{
	...
	if (g_str_equal(mode, "off")) {
		...
		if (read_on_mode(srcaddr, onmode, sizeof(onmode)) < 0 ||
			...
			strncpy(onmode, "connectable", sizeof(onmode) - 1);
...
int adapter_start(struct btd_adapter *adapter)
{
	...
	if ((!adapter->initialized && !main_opts.remember_powered) || err < 0) {
		if (!adapter->initialized && main_opts.mode == MODE_OFF)
			...
			strncpy(mode, "connectable", sizeof(mode) - 1);
	}
...
gboolean adapter_device_is_edr(bdaddr_t *local, uint16_t handle)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	...
	return device_is_edr(device);
}

int adapter_device_set_qos_role(bdaddr_t *local,
		uint16_t handle, uint8_t qos_role)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	...
	device_set_qos_role(device, qos_role);

	return 0;
}

int adapter_device_get_qos_role(bdaddr_t *local, uint16_t handle)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	...
	return device_get_qos_role(device);
}

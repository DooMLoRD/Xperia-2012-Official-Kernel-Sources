...
int write_local_class(bdaddr_t *bdaddr, uint8_t *class)
{
	...
	snprintf(str, sizeof(str), "0x%2.2x%2.2x%2.2x", class[2], class[1], class[0]);
...
int write_remote_class(bdaddr_t *local, bdaddr_t *peer, uint32_t class)
{
	...
	ba2str(peer, addr);
	snprintf(str, sizeof(str), "0x%6.6x", class);
					...
					uint8_t lmp_ver, uint16_t lmp_subver)
{
	...
	memset(str, 0, sizeof(str));
	snprintf(str, sizeof(str), "%d %d %d", manufacturer, lmp_ver, lmp_subver);
				...
				unsigned char *page1, unsigned char *page2)
{
	...
	if (page1)
		for (i = 0; i < 8; i++)
			snprintf(str + (i * 2), sizeof(str) - i * 2, "%2.2X", page1[i]);
	else if (old_value && strlen(old_value) >= 16)
		strncpy(str, old_value, 16);

	if (page2)
		for (i = 0; i < 8; i++)
			snprintf(str + 17 + (i * 2), sizeof(str) - 17 - i * 2, "%2.2X", page2[i]);
...
int write_link_key(bdaddr_t *local, bdaddr_t *peer, unsigned char *key, uint8_t type, int length)
{
	...
	memset(str, 0, sizeof(str));
	for (i = 0; i < 16; i++)
		snprintf(str + (i * 2), sizeof(str) - i * 2, "%2.2X", key[i]);
	snprintf(str + 32, sizeof(str) - 32, " %d %d", type, length);

...
void device_prepare_features(struct btd_device *device)
{
	bdaddr_t local;

	adapter_get_address(device_get_adapter(device), &local);
	read_remote_features(&local, &device->bdaddr, device->features, NULL);
}

gboolean device_is_edr(struct btd_device *device)
{
	return ((device->features[4] & LMP_EDR_3SLOT) ||
			(device->features[5] & LMP_EDR_5SLOT) ||
			(device->features[3] & LMP_EDR_ACL_2M) ||
			(device->features[3] & LMP_EDR_ACL_3M));
}

void device_set_qos_role(struct btd_device *device, uint8_t qos_role)
{
	device->qos_role = qos_role;
}

uint8_t device_get_qos_role(struct btd_device *device)
{
	return device->qos_role;
}
